...
import { handleGetAllGuides } from "../../Store/guide.slice";
...
export default function Guides() {
    ...
    const { guides, sourceFilters, classifications, yearFilters, numberOfPages, numberOfGuides } = useSelector((state: { guides: { guides: Guide[], sourceFilters: {source:string,count:string}[], classifications: {classification:string,count:string}[], yearFilters: {year:string,count:string}[], numberOfPages: number, numberOfGuides:number } }) => state.guides);
    ...
    const fetchData = async ({
        ...
        setIsLoading(true);
        try {
            const data = await dispatch(handleGetAllGuides({ page, classification, source, year, custom }));
      ...
      const handleKeyUp = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        localStorage.setItem("searchGuide", JSON.stringify(searchObject));
    ...
    useEffect(() => {
        window.scrollTo(0, 0);
        const searchObject = localStorage.getItem("searchGuide")? JSON.parse(localStorage.getItem("searchGuide") as string) : {};
      ...
      const handleNameChange = (value: string) => {
        ...
        localStorage.setItem("searchGuide", JSON.stringify(searchObject));
        fetchData({ classification: value, source: "", year: "", custom: searchValue });
      }
    ...
    return (
        <div className="my-5 container mx-auto mt-24 px-3">
          ...
          {/* Helmet */}
            <Helmet>
                <title> {`${SeoData?.guideTitle}`}</title>
                <meta name="description" content= {SeoData?.guideDescription} />
            </Helmet>
            ...
            <div className="bg-secondary_color mb-5 text-sm rounded-lg px-4 py-2">
                ...
                <span className="font-medium cursor-pointer">الأدلة المعرفية ({numberOfGuides})</span>
            </div>
