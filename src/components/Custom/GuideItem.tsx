...
import { handleDownloadGuide, resetDownloadProgress } from "../../Store/guide.slice";
...
export default function GuideItem({
  ...
  const handleDownload = async (pdfName: string) => {
    ...
    try {
      const data = await dispatch(handleDownloadGuide(pdfName));
      ...
      <div className={`grid grid-cols-12 gap-3 py-5 px-3 rounded-xl `}>
        ...
        <div className="col-span-5 sm:col-span-3 m-auto ps-2 gap-3 relative hidden sm:flex">
          ...
          >
            ...
            <div className="col-span-3">
            <a
            href={`/viewPdf/${pdfName}/Guides/${name.replace(/ /g, "-")}/${details
                .split(" ")
                .slice(0, 160)
                .join(" ")
                .replace(/ /g, "-")
